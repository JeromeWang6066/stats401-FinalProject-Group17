...
Visualization goals: a switchable definition; five views that share one filter and one selected event; a path from who, to when, to place change, to where, to what share of a race is real.

Audience: STATS401 reviewers, and F1 viewers told a season had many overtakes.

Questions: how much of the 2023-2025 feed sits in a start burst, a pit window, or a VSC / safety-car / red-flag interval; who is the net overtaking center after filtering, and whether edges pile into a few pairs; whether swaps cluster on lap number at starts and restarts; whether suspected true passes sit at particular corners; and, for one selected race, what share of OpenF1 swaps is on-track passing versus pit, retirement, neutralization, formation, or post-race.

## 2. Dataset(s)

**Source.** OpenF1, unofficial timing API, free from 2023. https://openf1.org/docs/. https://api.openf1.org/v1/.

**Acquisition.** Python writes API JSON under `data/raw/`. We list every `session_type=Race` weekend in 2023-2025 via `meetings` and `sessions`, then pull `overtakes`, `drivers`, `pit`, `race_control`, `laps`, and `position` by `session_key`. View 4 adds `location`, cut per session. View 5 aggregates the exclusive `tag` counts for that session.
...
**Size.** Three seasons, on the order of seventy races. Swap events should sit in the thousands, plus drivers, pits, race-control, position, and laps. `location` (~3.7 Hz) stays session-scoped.

**Attributes.** Overtaking and overtaken driver, timestamp, position; acronym and team colour; pit time; race-control flag and message; lap start; year and circuit; exclusive `tag`; later, `location` x/y.
...
Five techniques, different idioms, shared state: force-directed network; lap-number event strip; bump chart; spatial track map; race-composition donut. One path, not five separate charts.
...
5. **Race composition donut.** Exclusive `tag` shares for one selected session (`on_track`, `pit`, `retired`, `under_sc`, `unlap_under_sc`, `formation`, `post_race`). Center shows the session swap count and the on-track share. Clicking a slice sets the shared tag filter for the other views. Clicking `on_track` expands, in the same chart, the overlapping flags `lap1` / `burst` / `bounce` among true passes. Reference: part-to-whole donut with a count table. Shows whether a race's OpenF1 overtake total is mostly on-track passing or bookkeeping.
...
| Week 6 | Implementation & Refinement | `location` slices; per-session tag donut; five interactions | Tianyao Wang, Jietong Zhou | five-view draft |
